...
//*BOOL?
            ...
            | tless(n1: tm, n2: tm)              //                    (less-than relation on naturals)
//?BOOL*/
//?NAT*/
//*REC?
...
//*BOOL?
  ...
  case tless(n1, n2) => tless(subst(x, s, n1), subst(x, s, n2))
//?BOOL*/
//?NAT*/
//*REC?
...
//*BOOL?
  ...
  /* LessFalse0 */ else if (t.tless? && t.n1.tzero? && t.n2.tzero?) then
  Some(tfalse)
  /* LessFalse1 */  else if (t.tless? && t.n1.tsucc? && peano(t.n1) && t.n2.tzero?) then
  Some(tfalse)
  /* LessTrue2 */   else if (t.tless? && t.n1.tzero? && t.n2.tsucc? && peano(t.n2)) then
  Some(ttrue)
  /* LessRec */      else if (t.tless? && t.n1.tsucc? && t.n2.tsucc? && peano(t.n1) && peano(t.n2)) then
  Some(tless(t.n1.p, t.n2.p))
  /* Less1 */        else if (t.tless? && step(t.n1).Some?) then
  Some(tless(step(t.n1).get, t.n2))
  /* Less2 */        else if (t.tless? && peano(t.n1) && step(t.n2).Some?) then
  Some(tless(t.n1, step(t.n2).get))
//?BOOL*/
//?NAT*/
//*REC?
